...
- **[MODIFY]** `mod_root/manifest.sii` : ユーザー要望に基づき、アイコン画像の参照指定を削除して元のアイコンなし構成に復元。
- **[DELETE]** `mod_root/mod_icon.jpg` : アイコン用画像ファイルを削除。
- **[MODIFY]** `Plan.md` / `for_agent/requirements.md` : アイコン設定を「任意手順（Optional）」に変更・更新。
- **[MODIFY]** 一旦古いタグ `v1.0.1` とリリースをGitHubから削除し、不要ファイルのクリーンアップ後、新しくクリーンな状態でタグ `v1.0.1` を再作成してプッシュし、自動リリースのビルドをトリガー。
- **[DELETE]** `assets/` 内の不要なVOICEPEAK出力の連番一時ファイル（`00-ets2_voice_mod.wav`〜`39-ets2_voice_mod.wav` およびその `.txt` ファイル）をGitおよびローカルから完全削除。
- **[MODIFY]** 新しくリネーム・作成した複合/ショートWAVファイル（`compound_xxx.wav`、`xxx_short.wav` 等の14ファイル）をGitに追加してリモートに同期。
- **[MODIFY]** `for_agent/requirements.md` / `Plan.md` : ナビ音声の3パターン化（ランダム再生）および吃音・言い淀み表現の排除に対応した要件定義の更新。
- **[NEW]** `voicepeak_import_3patterns.txt` : VOICEPEAKでの一括音声合成用に、吃音を排除した120行分のセリフを並べたプレーンテキストを新規作成。
- **[NEW]** `voicepeak_file_mapping_3patterns.txt` : 連番出力ファイルを `assets/` 内の各3パターンWAV（`turn_left_1.wav` 等）に一括マッピングする定義ファイルを作成。
- **[NEW]** `scratch/rename_3patterns.ps1` : 連番WAVファイルを定義に沿って一括リネーム・配置するPowerShellスクリプトを作成。
- **[MODIFY]** `FMOD_INSTRUCTIONS.md` : FMOD Studioでランダム再生（Multi Sound）を設定し、3パターンの音声をアサインする手順・解説を追記。
